...
span_with_tag_like="<span class='glyphicon glyphicon-thumbs-up'>Like: "+like+"</span> </button><hr>";
row=name_with_tag + message_wth_tag + like_button + span_with_tag_like;
document.getElementById('output').innerHTML +=row;
//End code
    } });  }); }
getData();
function updateLike(message_id_like) { 
    console.log("clicked on like button - " + message_id_like); 
button_id_like = message_id_like;
 likes = document.getElementById(button_id_like).value;
  updated_likes = Number(likes) + 1;
   console.log(updated_likes);
    firebase.database().ref(roomname).child(message_id_like).update({ like : updated_likes }); 
}
function logout() {
